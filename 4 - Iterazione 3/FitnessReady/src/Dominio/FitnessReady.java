...
public class FitnessReady {
    ...
    //private perché il metodo viene chiamato una sola volta nel costruttore e mai più poiché FitnessReady è singleton
    private void setGestorePromozioni(){ gestorePromozioni = new GestorePromozioni(); }
    public GestorePromozioni getGestorePromozioni(){ return gestorePromozioni; }
    ...
    public void registraCliente(String nome, String cognome, String email, String password, String confermaPassword, String altezza, String peso, String obiettivo) throws ExistingEmailException, PasswordNotEqualsException, EmptyFieldException, NotPositiveNumberException {
        List<String> textList = new LinkedList<>(Arrays.asList(
                nome, cognome, email, password, confermaPassword, altezza, peso, obiettivo));
        int i = textList.size();
        float altezzaC, pesoC;
        ...
        if(i == textList.size()){
            ...
            if(password.equals(confermaPassword)){
                altezzaC = Float.parseFloat(altezza);
                pesoC = Float.parseFloat(peso);
                if(altezzaC >= 0 && pesoC >= 0){
                    Cliente newCliente = new Cliente(nome, cognome, email, password, altezzaC, pesoC, obiettivo);
    ...
    public void buyAndUpdate(Date dataEsecuzione, Date dataArrivo, String indirizzo, String città, String cap, String metodoPagamento, String numCarta, String cvv) throws GenericSystemException {
        ...
        List<Promozione> listaPromozioni = gestorePromozioni.getListaPromozioni();
        newAcquisto = gestoreAcquisto.effettuaAcquisto(metodoPagamento, listaPromozioni);
